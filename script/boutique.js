...
window.storeItemsData.forEach(item => item.basePrice = item.price);

const storeDiv = document.getElementById('storeItems');

function updateStore(){
  storeDiv.innerHTML='';
  window.storeItemsData.forEach((item,index)=>{
...
function acheterItem(index){
  const item=window.storeItemsData[index];
  if(count>=item.price){
    count-=item.price;
    ...
    flash.style.left=(itemDiv.offsetWidth/2-15)+'px'; flash.style.top=(itemDiv.offsetHeight/2-15)+'px';
    itemDiv.appendChild(flash); setTimeout(()=>flash.remove(),500);
...
function chargerJeu(){
  ...
  if(saved){
    ...
    data.storeItems.forEach((itemSaved,i)=>{
      window.storeItemsData[i].owned=itemSaved.owned;
      window.storeItemsData[i].price=itemSaved.price;
    });
  }
}
...
function sauvegarderJeu(){
  const saveData={
    ...
    storeItems: window.storeItemsData.map(item=>({owned:item.owned,price:item.price}))
  };
